...
def Algm():
    def success_rate(soln):
        ...
        for i in range(len(soln)):
            S = np.array(soln[i]) - np.array(XL)
            f = 1 / norm(S)
            fit.append(f)
        return fit

    X= np.array([[0.8147,0.9058,0.1270],    # X array
                [0.9134,0.6324,0.0975],
                [0.2785,0.5469,0.9575],
                [0.9649,0.1576,0.9706]])
    Tmax,R,Q = 500,4,3               #values of Tmax,R,Q
    B,F,O,A = R/4,R/4,R/4,R/4      #values of B,F,O,A

    Te=[]                          #finding the Teta
    for i in range(R):
        Te.append(((i+1)*360)//R)
    #for i in range(len(Te)): print(Te[i])
    ...
    #Loop Begins
    #print("Iteration    Best Fitness    Leading rider")
    while(g<Tmax):
        ...
        l1 = (X[len(B) + len(F)]) - (XL)  # To find the threshold of the overtaker(here --->X[len(B)+len(F)] denotes(X[2])
        ...
        for i in range(R):
            if ((A1[i] == 1) and (E[i] != Emod)):
                E1[i] += 1
            elif ((A1[i] == 0) and (E[i] != 0)):
                E1[i] = E[i] - 1
            else:
                E1[i] = E[i]
        # print(E1)
